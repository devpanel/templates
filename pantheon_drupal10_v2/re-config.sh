...
PANTHEON_DEV_STATIC_FILES_PATH="$WEB_ROOT/sites/default/files/files_dev"
PANTHEON_TEST_STATIC_FILES_PATH="$WEB_ROOT/sites/default/files/files_test"
PANTHEON_LIVE_STATIC_FILES_PATH="$WEB_ROOT/sites/default/files/files_live"
...
#== Composer install.
sed -i -e "s/\"classmap\": \[\"upstream-configuration\/scripts\/ComposerScripts\.php\"\]//g" $APP_ROOT/composer.json
composer remove pantheon-systems/drupal-integrations  pantheon-upstreams/upstream-configuration
...
if [[ $(mysql -h$DB_HOST -P$DB_PORT -u$DB_USER -p$DB_PASSWORD $DB_NAME -e "show tables;") == '' ]]; then
  if [[ -f "$APP_ROOT/.devpanel/dumps/files.tgz" ]]; then
    ...
    if [ -d "$PANTHEON_DEV_STATIC_FILES_PATH" ]; then
      sudo mv "${PANTHEON_DEV_STATIC_FILES_PATH}"/{.,}* "$STATIC_FILES_PATH"
    fi
    if [ -d "$PANTHEON_TEST_STATIC_FILES_PATH" ]; then
      sudo mv "${PANTHEON_TEST_STATIC_FILES_PATH}"/{.,}* "$STATIC_FILES_PATH"
    fi
    if [ -d "$PANTHEON_LIVE_STATIC_FILES_PATH" ]; then
      sudo mv "${PANTHEON_LIVE_STATIC_FILES_PATH}"/{.,}* "$STATIC_FILES_PATH"
    fi
  fi
